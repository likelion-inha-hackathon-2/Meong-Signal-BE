...
class LocationConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def receive_json(self, content):
        try:
            ...
            await self.save_location(room, latitude, longitude, walk_user_id)
    ...
    def get_or_create_room(self, walk_user_id, owner_id):
    ...
    @database_sync_to_async
    def save_location(self, room, latitude, longitude, walk_user_id):
        # 위, 경도가 제공되었는지 확인합니다.
        if not latitude or not longitude or not walk_user_id:
            raise ValueError("위, 경도 정보가 필요합니다.")
        
        # 위치 정보를 생성하고 데이터베이스에 저장합니다.
        Location.objects.create(room=room, walk_user_id=walk_user_id, latitude=latitude, longitude=longitude)
